# HELLDIVERS 2 Bot — Super Terra

Posta automaticamente no canal `#contato-super-terra` do Discord, via webhook (`HELLDIVERS Stats`):

- **Contagem de jogadores online** do HELLDIVERS 2 na Steam
...
- **Notícias/patch notes oficiais** do HELLDIVERS 2 (só posts reais da Arrowhead Game Studios na Steam — não pega matéria de imprensa/fansite)
...
Roda 100% no GitHub Actions — depois de configurado, funciona sozinho, sem precisar de nada ligado no computador de ninguém. Mesmo modelo dos bots do ARC Raiders e do WARDOGS.

## Sobre PlayStation

A Steam tem uma API pública e gratuita de contagem de jogadores online, mas o **PSN não oferece nenhuma API pública equivalente** para jogos de terceiros — só a própria Sony (ou a Arrowhead, internamente) tem acesso a esse número. Não existe hoje uma forma confiável de puxar "jogadores ativos no PS5" de fora sem inventar o dado. Por isso este bot traz apenas o número oficial da Steam.

## Como funciona por baixo dos panos

- **Steam Web API** (pública, sem chave): número de jogadores online (`ISteamUserStats/GetNumberOfCurrentPlayers`) e notícias (`ISteamNews/GetNewsForApp`), App ID do HELLDIVERS 2: `553850`.
...
Os horários evitam cair exatamente em hora cheia (ex: `22:00`), porque o GitHub avisa que agendamentos assim são os mais sujeitos a atraso ou até serem descartados (é quando mais workflows do mundo inteiro tentam rodar ao mesmo tempo). Por isso usamos `22:05` em vez de `22:00`, por exemplo. São os mesmos horários usados nos bots do ARC Raiders e do WARDOGS.
...
- `scripts/post-news.js` — busca notícias oficiais do HELLDIVERS 2, traduz pra português e posta as que ainda não foram postadas (ou a mais recente, se rodado manualmente).
- `scripts/state.json` — guarda quais notícias já foram postadas (evita repetir). Atualizado automaticamente.
- `.github/workflows/helldivers-player-count.yml` — agendamento da contagem de jogadores (diário + fim de semana).
- `.github/workflows/helldivers-news.yml` — agendamento da checagem de notícias (a cada 30 min).

## Configuração (caso precise refazer do zero)

### 1. Repositório no GitHub

Pode ser público ou privado — o Secret do webhook fica protegido de qualquer forma.

### 2. Subir os arquivos

Pelo site do GitHub:
- Pastas/arquivos comuns → **Add file → Upload files** (arrastar).
- A pasta `.github/workflows` não sobe direito arrastando — para cada arquivo `.yml`, use **Add file → Create new file** e digite o caminho completo no nome (ex: `.github/workflows/helldivers-player-count.yml`), o GitHub cria as pastas sozinho.

### 3. Configurar o Secret do webhook

**Settings → Secrets and variables → Actions → New repository secret**
- Nome: `DISCORD_WEBHOOK_URL`
- Valor: a URL do webhook "HELLDIVERS Stats" (Discord → canal `contato-super-terra` → Editar canal → Integrações → Webhooks)
...
- **Avatar do bot**: Discord → canal → Editar canal → Integrações → Webhooks → clique no avatar do "HELLDIVERS Stats" → escolher imagem.
